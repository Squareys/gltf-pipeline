 ...
 */
function palette_to_image(filename) {
    var png = new PNG({
    ...
    if(filename) fsExtra.writeFileSync(filename, data);
    return data;
}
...
function getWorldTransform(gltf, node, nodeId) {
    ...
    if(parent) {
        mat4.multiply(mat, getWorldTransform(gltf, gltf.nodes[parent], parent), mat);
    }
...
function toPaletteMesh(gltf) {
    ...
    gltf.nodes.forEach(function(node, i) {
        if(node.mesh != undefined) {
            ...
            ForEach.meshPrimitive(mesh, function(primitive, i) {
                var mat = gltf.materials[primitive.material];
                if(!mat.pbrMetallicRoughness || mat.alphaMode == 'BLEND') return;

                var i = palette_add(gltf
                    .materials[primitive.material]
                    .pbrMetallicRoughness
                    .baseColorFactor);

                var mat = getWorldTransform(gltf, node, i);
                ...
                mergedPrimitives.push(i);
            });
            if(mergedPrimitives.length == mesh.primitives.length) {
                delete node.mesh;
            } else {
                mergedPrimitives.reverse().forEach(i => delete mesh.primitives[i]);
            }
    ...
    /* Merge buffers and add texture coords */
    ...
    var paletteData = palette_to_image();
